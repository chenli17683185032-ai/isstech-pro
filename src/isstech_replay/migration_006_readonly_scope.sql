...
CREATE TABLE readonly_module_runs_v6 (
    run_id TEXT PRIMARY KEY,
    module TEXT NOT NULL CHECK (module IN ('payment', 'bizcase', 'travel_application')),
...
INSERT INTO readonly_module_runs_v6
SELECT * FROM readonly_module_runs;

CREATE TABLE readonly_module_snapshots_v6 (
    snapshot_id INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id TEXT NOT NULL REFERENCES readonly_module_runs_v6(run_id) ON DELETE RESTRICT,
    module TEXT NOT NULL CHECK (module IN ('payment', 'bizcase', 'travel_application')),
...
INSERT INTO readonly_module_snapshots_v6
SELECT * FROM readonly_module_snapshots;

CREATE TABLE readonly_module_current_v6 (
    module TEXT NOT NULL CHECK (module IN ('payment', 'bizcase', 'travel_application')),
    external_id TEXT NOT NULL CHECK (length(external_id) > 0),
    first_seen_at TEXT NOT NULL,
    last_seen_at TEXT NOT NULL,
    last_run_id TEXT NOT NULL REFERENCES readonly_module_runs_v6(run_id) ON DELETE RESTRICT,
    payload_json TEXT NOT NULL,
    payload_hash TEXT NOT NULL CHECK (length(payload_hash) = 64),
    PRIMARY KEY (module, external_id)
);

INSERT INTO readonly_module_current_v6
...
ALTER TABLE readonly_module_runs_v6 RENAME TO readonly_module_runs;
ALTER TABLE readonly_module_snapshots_v6 RENAME TO readonly_module_snapshots;
ALTER TABLE readonly_module_current_v6 RENAME TO readonly_module_current;
...
CREATE TABLE readonly_scope_assertions (
    module TEXT NOT NULL CHECK (module IN ('payment', 'bizcase', 'travel_application')),
    external_id TEXT NOT NULL CHECK (length(external_id) > 0),
