...
并完成离线详情、current 对账和浏览器令牌恢复；实现与真实 QA 门禁均已通过，
当前只等待记录阶段性提交；
...
npm production build: 1593 modules, JS 243.20 kB, CSS 29.52 kB
real read-only scope: Search candidates=78, participant matches=2, approved=2
real relations: applicant=1, submitter=2, project_manager=2
real cache: current=2, payload_v2=2, cached_detail=2
desktop Browser QA: 1280x720, approved rows=2, 14 fields each, approval steps=6/9
mobile Browser QA: 390x844, page overflow=false, full-width local detail drawer
session QA: backend restart requires login; subsequent reload remains authenticated
console QA: no application warning/error across list, both details and reload
wheel: 60 files; work-item relations, cached detail, web_dist and storage invariant present
...
| 26 | `IN_PROGRESS` | P9.4 阶段性提交 | 全量门禁已通过，等待记录提交 ID |
