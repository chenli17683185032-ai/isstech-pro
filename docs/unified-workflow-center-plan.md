...
P9.4 正在将账号口径从单一“申请人精确相等”修正为已由 Detail 证明的本人参与关系，
并修复详情临时上游读取和浏览器标签重建后丢失令牌的问题；
P7 保持 `BLOCKED`，P10 保持 `TODO`。
...
npm production build: 1593 modules, JS 241.91 kB, CSS 28.70 kB
real read-only scope: Search candidates=78, applicant matches=1, approved=1
real owned detail: 14 displayed fields, 14 populated, 9 approval steps
default view QA: follow_up=0, approved=1 -> automatic approved, row=1
manual view QA: select follow_up -> read-only sync -> manual follow_up preserved
desktop Browser QA: 1440x900, local drawer, no console error/overflow/overlap
mobile Chrome QA: 390x844, full-screen detail, no console error/overflow/overlap
Escape/close QA: approved filter and row preserved, focus restored
wheel: 60 files; work-item detail route, web_dist and storage invariant present
