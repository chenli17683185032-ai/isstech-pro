...
状态：当前执行节点（唯一状态以“当前执行清单”第 37 项为准）
...
| 37 | `IN_PROGRESS` | P9.8 Payment/Bizcase 运行态协议取证 | 双 URL 只读链路 + schema + 分页/详情决策 |
| 38 | `TODO` | P9.8 安全策略、模型和解析器 | 精确 allowlist + 独立固定 schema + 写路径全阻断 |
